      ...
      <div className="relative" style={{ zIndex: 10 }}>
        ...
        <footer id="docs" style={{ borderTop: `1px solid ${C.line}`, background: "rgba(11,14,12,0.6)", backdropFilter: "blur(8px)" }}>
          <div className="mx-auto px-6 py-12 md:py-16 grid md:grid-cols-4 gap-10" style={{ maxWidth: 1150 }}>
            ...
            {[
              ["Protocol", [["How it works", "/#protocol"], ["Vendor registry", "/registry"], ["Documentation", "/docs"], ["Network metrics", "/metrics"]]],
              ["Open source", [["GitHub", "https://github.com/hoodwire/hoodwire"], ["MCP integration", "/docs"], ["Registry spec", "/registry"], ["Settlement contracts", "https://github.com/hoodwire/hoodwire/tree/main/contracts"]]],
              ["Connect", [["X / Twitter", "https://x.com/hoodwirexyz"]]],
            ].map(([h, links]) => (
              <div key={h}>
                <div className="text-xs uppercase tracking-widest mb-4" style={{ color: C.ink }}>{h}</div>
                <ul className="space-y-2.5 text-sm">
                  {links.map(([l, href]) => (
                    <li key={l}><a href={href} className="hw-link">{l}</a></li>
                  ))}
                </ul>
              </div>
            ))}
